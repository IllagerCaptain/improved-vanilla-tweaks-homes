...
public class SethomeCommandExecutedProcedure {
	public static void executeProcedure(Map<String, Object> dependencies) {
		...
		if (dependencies.get("cmdparams") == null) {
			System.err.println("Failed to load dependency cmdparams for procedure SethomeCommandExecuted!");
			return;
		}
		...
		if ((new Object() {
		...
		}.getText())) >= 1)) {
			{
				...
				if (!_ent.world.isClient()) {
					_ent.world.getServer().getCommandManager().execute(_ent.getCommandSource().withSilent().withLevel(4),
							(("trigger sethome set ") + "" + ((new Object() {
								public String getText() {
									String param = (String) cmdparams.get("0");
									if (param != null) {
										return param;
		...
		}.getText())).equals(""))) {
			{
				Entity _ent = entity;
				if (!_ent.world.isClient()) {
					_ent.world.getServer().getCommandManager().execute(_ent.getCommandSource().withSilent().withLevel(4), "trigger sethome");
				}
			}
		}
	}
